...
export function CandleDetailPage() {
  ...
  // Variant selection state (Candles might use Scent as variant if grouped, but here we use Size if available or just show logic)
  // Assuming product might have 'colors' (scents?) or we can add 'sizes' later. For now sticking to 'colors' prop or just fallback.
  // Since candles have 'size' property, we could treat that as a variant if we had multiple sizes for same ID (we don't). 
  // We'll stick to 'colors' if present (maybe for jar color?) or just hide selector.
  const [selectedColor, setSelectedColor] = useState<string>(
    product?.colors?.[0] || ""
  );

  if (!product || product.category !== "candle") {
    return (
      <Section>
        <div className="text-center py-8 md:py-12">
          <h1 className="text-xl md:text-2xl font-semibold mb-3 md:mb-4">Product not found</h1>
          <Link to="/candles">
            <Button variant="outline" className="text-sm md:text-base">Back to Candles</Button>
          </Link>
        </div>
      </Section>
    );
        ...
        <div className="space-y-4 md:space-y-6">
          ...
          {product.colors && product.colors.length > 0 && (
            <div className="space-y-3 pt-2">
              <span className="text-sm font-medium text-muted-foreground">Select Option:</span>
